  ...
  useEffect(() => {
    ...
    const initService = async () => {
      try {
        ...
        console.log("[EditIdentityPage] Initializing blockchain service for:", selectedChain);
        const blockchainService = new BlockchainService();
        await blockchainService.initialize(selectedChain);
        console.log("[EditIdentityPage] Blockchain service initialized successfully");
        const service = new IdentityService(blockchainService);
        setIdentityService(service);
      } catch (error) {
        console.error("[EditIdentityPage] Failed to initialize identity service:", error);
        ...
        try {
          ...
          console.error("[EditIdentityPage] Fallback initialization also failed:", fallbackError);
        }
      } finally {
        setLoading(false);
      }
