...
interface WalletStateContextValue {
    ...
    error: Error | null;
    // Wallet-agnostic async methods
    signTransaction: (tx: any) => Promise<any>;
    signMessage: (message: string) => Promise<string>;
    getWalletType: () => 'private' | 'managed' | null;
    // Service instance for blockchain operations
    service: any;
}
...
export const useBlockchainManager = () => {
    ...
    if (!context) {
        // Return a default state when not within BlockchainSelectionManager
        return {
            isConnected: false,
            account: null,
            signTransaction: async () => { throw new Error('BlockchainSelectionManager not initialized'); },
            signMessage: async () => { throw new Error('BlockchainSelectionManager not initialized'); },
            service: null,
            getWalletType: () => null
        };
    }
    return {
        ...
        getWalletType: context.getWalletType
    };
};
...
const ChainInfoTab = ({ chainConfig, isDev, React }: { chainConfig: any; isDev: boolean; React: any }) => {
...
const DiamondLoupeTab = ({
    ...
    diamondLoupeInfo: any;
    loadingLoupe: boolean;
    React: any;
}) => {
    if (loadingLoupe) {
        return (
...
export const BlockchainSelectionManager = ({
    ...
    const [chainConfigs, setChainConfigs] = useState<ChainConfigs | null>(null);
    const [showInfoModal, setShowInfoModal] = useState(false);
    const [activeModalTab, setActiveModalTab] = useState('info');
    const [diamondLoupeInfo, setDiamondLoupeInfo] = useState<any>(null);
    ...
    useEffect(() => {
        const initService = async () => {
            ...
            try {
                ...
                console.log(`[BlockchainSelectionManager] Initialized blockchain service for ${selectedChainId}`);
            } catch (error) {
    ...
    const walletStateContextValue: WalletStateContextValue = {
        ...
        walletProvider: walletState.walletProvider,
    ...
    return (
        <WalletStateContext.Provider value={walletStateContextValue}>
        <>
