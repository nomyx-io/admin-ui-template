    ...
    useEffect(() => {
		...
		(async function () {
            ...
            if(service.getClaimTopics && service.getTrustedIssuersByObjectId){
                ...
                let newArr = []
                const claimTopics =  issuerData?.attributes?.claimTopics.forEach((item)=>{newArr.push(item.topic)})
    ...
    const updateTrustedIssuer = ()=> {
        if (validateTrustedIssuer(verifierName.trim(), walletAddress, targetKeys)) {
            toast.promise(
                () => {
                    let keys = []
                    targetKeys.map((item)=>{ keys.push({"topic": item, "timestamp": Date.now()})})
                    return service.updateIssuerClaimTopics(walletAddress,targetKeys)
                    .then((event) => {
    ...
    return (
        <div >
            ...
            <div className='p-6 mt-2'>
                <div>
                    <label for="trustedIssuerName">Trusted Issuer display name *</label>
                    <div className='mt-3 relative w-full flex border rounded-lg'>
                        <Input id="trustedIssuerName" value={verifierName} className='border w-full p-2 rounded-lg text-xl' placeholder='ID Verifier Name' type='text' maxLength={32} onChange={(e) => setVerifierName(e.target.value)} />
                        <p className='absolute right-5 top-3'>{verifierName.length}/32</p>
                    </div>
                </div>
                <div className='mt-10 mb-6'>
                    <label for="trustedIssuerWallet">Trusted Issuer Wallet *</label>
                    <div className='mt-3 relative w-full flex border rounded-lg'>
                        <Input id="trustedIssuerWallet" value={walletAddress} className='border w-full p-2 rounded-lg text-xl' placeholder='Wallet Address' type='text' onChange={(e) => id == 'create' ? setWalletAddress(e.target.value.trim()) : ''} />
                    </div>
                ...
                {id == 'create' ?<Button className='max-[600px]:w-[60%] min-w-max text-center font-semibold rounded h-11 bg-[#7F56D9] text-white' onClick={saveTrustedIssuer}>Create Trusted Issuer</Button>
                 : <Button className='max-[600px]:w-[60%] min-w-max text-center font-semibold rounded h-11 bg-[#7F56D9] text-white' onClick={updateTrustedIssuer}>Update Trusted Issuer</Button>}
                </div>
            </div>
        </div>
    )
