...
import { DfnsApiClient } from '@dfns/sdk-browser';
...
export const FixAuth = ({ dfnsToken }: { dfnsToken: string | null }) => {
  ...
  const handleFixAuth = async () => {
    ...
    try {
      const dfnsBrowserClient = new DfnsApiClient({
        ...
        baseUrl: process.env.REACT_APP_DFNS_API_URL || 'https://api.dfns.io',
        authToken: dfnsToken,
      });
      ...
      const feeSignedChallenge = await dfnsBrowserClient.signUserActionChallenge(feeInit.challenge);

      await Parse.Cloud.run('dfnsSetFeeControllerComplete', {
        walletId,
        requestBody: feeInit.requestBody,
        signedChallenge: feeSignedChallenge,
        dfns_token: dfnsToken,
      });
      ...
      const issuerSignedChallenge = await dfnsBrowserClient.signUserActionChallenge(issuerInit.challenge);

      await Parse.Cloud.run('dfnsAddTrustedIssuerComplete', {
        walletId,
        requestBody: issuerInit.requestBody,
        signedChallenge: issuerSignedChallenge,
        dfns_token: dfnsToken,
      });
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md max-w-lg mx-auto mt-10">
      ...
      <p className="mb-4 text-gray-600">This will update the Fee Controller and add the owner as a Trusted Issuer. You will be prompted to sign two WebAuthn challenges.</p>
      <div className="mb-4">
