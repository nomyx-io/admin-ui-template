...
const TrustedIssuersPage = ({ service }) => {
    ...
    async function fetchData() {
        ...
        if (issuers) {
            issuers.forEach((item) => {
                ...
                data.push({ id: item.id, claimTopics: claimTopicsString, address: item.attributes.issuer, trustedIssuer: item.attributes.verifierName })
            });
    ...
    const actions = [
        ...
        { label: "Remove", name: NomyxAction.RemoveTrustedIssuer,confirmation: "Are you sure you want to remove this Trusted Issuer?" }
    ];
    const globalActions = [
        { label: "Create Trusted Issuer", name: NomyxAction.CreateTrustedIssuer }
    ];
